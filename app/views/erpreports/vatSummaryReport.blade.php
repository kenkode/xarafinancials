...
<div class="footer">
   ...
   <div align="center"><strong>VAT Summary Report as from:  {{$from}} To:  {{$to}}</strong></div><br>
    ...
    <table class="table table-bordered" border='1' cellspacing='0' cellpadding='0'>
      ...
      <tr>
       ...
       <td td width='20' valign="top">{{$i}}</td>
        <td valign="top"> {{ $vat->order_number }}</td>
        <td valign="top"> {{ date("d/m/Y",strtotime($vat->created_at)) }}</td>
        <td valign="top" align="right"> {{ asMoney($vat->amount) }}</td>
        </tr>
      <?php
       $total = $total + $vat->amount;
       $i++; ?>
