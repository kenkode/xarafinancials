...
public function purchases(){
  ...
  $organization = Organization::find(Confide::user()->organization_id);
   ...
   public function pricelist(){
    ...
    public function vatselect()
    {
       return View::make('erpreports.vatSelect');
    }
    ...
    public function vatsummary(){
        ...
        $vats = DB::table('tax_orders')
                    ->join('taxes','tax_orders.tax_id','=','taxes.id')
                    ->where('taxes.name','VAT')
                    ->where('tax_orders.organization_id',Confide::user()->organization_id)
                    ->whereBetween('tax_orders.created_at', array(Input::get("from"), Input::get("to")))->get();

        $organization = Organization::find(Confide::user()->organization_id);

        $pdf = PDF::loadView('erpreports.vatSummaryReport', compact('vats', 'organization','from','to'))->setPaper('a4')->setOrientation('potrait');
    
        return $pdf->stream('VAT Summary List.pdf');
